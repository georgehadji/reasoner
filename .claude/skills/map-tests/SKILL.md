...
| `test_ic_jev_critic` | 1 | Iterative Critique with jev as first critic, offline: a jev ACCEPT (all four dims ≥ `ACCEPT_SCORE`) skips the LLM critic and ends the loop; any dim below, failure, timeout, or over-long answer runs the LLM critic unchanged; shadow runs both; the prompt and the jev path share one ACCEPT rule. |
