...
| `capabilities/page.tsx` | Re-exports `components/landing/CapabilitiesPage` with its metadata — the mechanism argument (§1–§9) that used to run down the home page. |
| `developers/page.tsx` | Re-exports `components/landing/DevelopersPage`, plus the `WebAPI` / `HowTo` / breadcrumb JSON-LD that makes the MCP setup quotable by an answer engine. |
...
| `landing/` | `LandingPage.tsx` (the claim, the rail, and the image/article exhibits), `CapabilitiesPage.tsx` (the nine mechanism sections, served at `/capabilities`), `DevelopersPage.tsx` (the programmatic surface in seven sections, served at `/developers` — MCP, HTTP, and CLI as three doors onto one pipeline; every tool name and endpoint in it must be checked against `src/reasoner/api/`), `prose.tsx` (the `Section`/`Heading`/`Lede`/`Body`/`Aside` chrome both share — edit it once or the two pages stop reading as one document) and `Testimonial.tsx` (renders nothing while empty, by design). The masthead and mechanism visuals, each drawing the product's own claim rather than decorating it: `DisagreementField.tsx` (`'use client'` canvas behind the hero — four point groups orbit one attractor and never reach it), `MechanismDiagram.tsx` (the four failure modes as stops on a rail, HTML not SVG so labels keep the page's point scale and reflow on a phone), `PipelineRail.tsx` (`'use client'` canvas over that rail — packets cross the four stages in lanes that never merge), `CollagePlate.tsx` (server-rendered SVG, one square plate per failure mode, seeded PRNG at module load so the markup is deterministic and costs no runtime JS). |
