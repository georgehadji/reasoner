# Graph Report - Reasoner  (2026-06-11)

## Corpus Check
- 2749 files · ~2,689,162 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 31928 nodes · 39318 edges · 2488 communities (2402 shown, 86 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 4822 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f31cc437`
...
1. `PipelineState` - 520 edges
2. `WorkflowServices` - 158 edges
3. `ProviderRouter` - 143 edges
4. `DomainEvent` - 103 edges
5. `extract_json()` - 99 edges
6. `get_language_instruction()` - 96 edges
7. `PhaseStep` - 88 edges
8. `WorkflowStrategy` - 84 edges
9. `str` - 83 edges
10. `User` - 78 edges

## Surprising Connections (you probably didn't know these)
- `test_encryption()` --calls--> `EncryptionService`  [INFERRED]
  verify_encryption.py → scripts/migrate_encryption_v2.py
- `MockSearchClient` --uses--> `PipelineState`  [INFERRED]
  .tmp_manual_temp/run_prism_tests.py → src/reasoner/domain/pipeline_state.py
- `MockSearchClient` --uses--> `SearchServicePort`  [INFERRED]
  .tmp_manual_temp/run_prism_tests.py → src/reasoner/core/ports/search_port.py
- `MockSearchClient` --uses--> `PrismClassification`  [INFERRED]
  .tmp_manual_temp/run_prism_tests.py → src/reasoner/application/services/prism_classifier.py
- `test_classify_query_parsing()` --calls--> `classify_query()`  [INFERRED]
  .tmp_manual_temp/run_prism_tests.py → src/reasoner/application/services/prism_classifier.py

## Communities (2488 total, 86 thin omitted)

### Community 0 - "Article Pipeline"
Cohesion: 0.03
Nodes (58): PipelineState, Get condensed decomposition summary for token efficiency., Get condensed candidates summary for token efficiency., Serialize complete state to dictionary (for persistence)., Save state to JSON file.                  Args:             path: File path t, Complete pipeline state — passed between phases., Initialize missing dataclass fields for backward-compatible --resume loading., Compatibility layer for old handler code expecting a dict. (+50 more)

### Community 1 - "Authentication & Security"
Cohesion: 0.06
Nodes (83): extract_json(), Extract JSON object (dict) from LLM response.     Raises ParseError if no objec, Brainstorming phase logic., run_brainstorm_cluster_phase(), run_brainstorm_develop_phase(), run_brainstorm_generate_phase(), run_brainstorm_synthesis_phase(), run_coding_assemble_phase() (+75 more)

### Community 2 - "Event Sourcing & Domain Events"
Cohesion: 0.02
Nodes (137): HistoryEntry, _list_history(), List history entries, sorted by timestamp descending.      If user_id is provi, Save a history entry to disk., _save_history_entry(), get_critical_phases(), get_phase_start_models(), Phase execution helpers extracted from streaming.py.  Contains:   - _PHASE_ROLE_ (+129 more)

### Community 3 - "Verbalized Sampling & Benchmarks"
Cohesion: 0.04
Nodes (73): ClaimLabel, Reasoner Pipeline - Output Renderer Rich terminal display and JSON export, with, Dispatch to the appropriate method-specific renderer., render_pipeline_result(), Namespace, cmd_list_models(), main(), parse_args() (+65 more)

### Community 4 - "Neuro Memory & LTM"
Cohesion: 0.05
Nodes (102): check_preset_access(), check_quota(), check_quota_if_authenticated(), check_rate_limit(), _ensure_user_in_db(), get_current_user(), get_event_bus(), get_optional_user() (+94 more)

### Community 5 - "Widgets & Calculator"
Cohesion: 0.08
Nodes (110): _parse_critique_scores(), ParseError, Raised when LLM response cannot be parsed into expected structure., Safely build CritiqueScore objects from raw LLM output.      CritiqueScore has, PhaseResult, Immutable record of one phase's execution.     Accumulated in PipelineState.pha, Decomposition, CriticDimensionScore (+102 more)
...
Cohesion: 0.04
Nodes (74): create_checkout(), create_portal(), _get_paypal_service(), _get_stripe_service(), get_subscription(), paypal_webhook(), FastAPI router for all billing endpoints., Public endpoint for PayPal webhooks. (+66 more)

### Community 8 - "Redis & Caching"
Cohesion: 0.07
Nodes (62): clear_memory_cache(), Clear the in-memory cache (thread-safe)., clear_cache(), _filter_routing(), _metrics_ip_restricted(), Drop routing entries whose API key is missing; fall back to primary., Wrap run_stream_cached with Prometheus metrics., Backward-compat gate: require auth when legacy API key mode is disabled. (+54 more)

### Community 9 - "Image Generation & Encryption"
Cohesion: 0.10
Nodes (22): PayPal implementation of BillingPort for one-time and subscription payments., datetime, Cache-aside decorator for QuotaRepository.  Redis caches hot quota reads (TTL 60, Admin endpoints — manual operations requiring ADMIN_API_KEY authentication., Raise 403 unless request carries a valid X-Admin-Key header., Manually trigger event store compaction.      Use ?dry_run=true to count eligibl, _require_admin(), trigger_compaction() (+14 more)

### Community 11 - "Application Entry Points"
Cohesion: 0.08
Nodes (92): Aggregate, PipelineStateData, Reasoner - Event-Sourced Aggregates  Aggregates are the core transactional bou, Get current state as dictionary., Data class representing pipeline state., Serialize state to dictionary., Deserialize from dictionary., Data class representing widget state. (+84 more)

### Community 12 - "Subagent Framework"
Cohesion: 0.05
Nodes (47): Serialize event to dictionary for storage., Deserialize event from dictionary., SaaS / billing events., SaaSEventType, Alert, AlertConfig, AlertManager, AlertNotifier (+39 more)
...
Cohesion: 0.18
Nodes (16): benchmark_vs_calibration(), main(), _pearson_r(), float, Calibration benchmark: verbalized entropy vs uncertainty correlation., Simulate entropy vs uncertainty measurements., benchmark_vs_diversity(), main() (+8 more)

### Community 17 - "LLM Exceptions & Errors"
Cohesion: 0.06
Nodes (37): AST, CodebaseIntrospector, ComplexityLevel, DeadCodeItem, DependencyEdge, ErrorHandlingGap, FunctionInfo, generate_markdown_report() (+29 more)
...
Cohesion: 0.05
Nodes (43): EventBus, get_event_bus(), handle_all_events(), handle_event(), init_default_subscribers(), log_all_events(), persist_all_events(), Application Layer - Event Bus  The event bus distributes domain events to subs (+35 more)

### Community 22 - "Chat UI Components"
Cohesion: 0.13
Nodes (41): PhaseQualityResult, _check_brainstorm_cluster(), _check_brainstorm_develop(), _check_brainstorm_generate(), _check_classification(), _check_context_vetting(), _check_critique(), _check_decompose_topic() (+33 more)

### Community 23 - "JSON Parsing Utilities"
Cohesion: 0.03
Nodes (142): ArticleFlow, Article reasoning workflow strategy., Article workflow:     1. Retrieve Sources     2. Draft     3. Adversarial Ver, PhaseStep, Base interfaces for workflow strategies., A single step in a reasoning flow., Port defining core services provided by the orchestrator to workflows., Protocol for reasoning workflow strategies. (+134 more)

### Community 24 - "Event Store Tests"
Cohesion: 0.12
Nodes (19): check_provider_health(), get_health_checker(), get_system_health(), HealthCheckResult, HealthStatus, ProviderHealth, ProviderHealthChecker, Any (+11 more)
...
Cohesion: 0.09
Nodes (23): create_limiter(), get_rate_limiter(), is_slowapi_available(), Any, bool, float, int, str (+15 more)

### Community 29 - "Quality Gates"
Cohesion: 0.14
Nodes (20): AuditEvent, AuditEventType, AuditLogger, AuditTimer, get_audit_logger(), log_api_request(), log_api_response(), log_pipeline_event() (+12 more)
...
Cohesion: 0.12
Nodes (31): Popen, _docker_available(), _import_smoke_test(), main(), _port_in_use(), print_banner(), bool, float (+23 more)

### Community 32 - "Event Store Count Tests"
Cohesion: 0.13
Nodes (37): FileSearchPort, _action_scrape(), _action_uploads_search(), _action_web_search(), _build_iteration_prompt(), _Citation, _cosine_similarity(), _map_step_type() (+29 more)

### Community 33 - "Event Store List Tests"
Cohesion: 0.50
Nodes (3): Background task handlers for periodic maintenance., Reset all usage_quotas at month start. Called by external scheduler., reset_all_quotas_monthly()

### Community 34 - "Pricing & Cost Estimation"
Cohesion: 0.10
Nodes (29): GetHistoryEntryQuery, GetHistoryQuery, GetModelQuery, GetPipelineHistoryQuery, GetPipelineStatusQuery, GetPresetQuery, GetProviderHealthQuery, GetWidgetStateQuery (+21 more)

### Community 35 - "Event Store Get Tests"
Cohesion: 0.03
Nodes (74): ChatFeed, ChatFeedMessage, ChatFeedProps, ContinueButton(), MessageActions(), RenderedPhase, ChatMessage, ChatMessageProps (+66 more)

### Community 36 - "Event Store Stats Tests"
Cohesion: 0.11
Nodes (28): clear_buffers(), emit_metric(), end_span(), _execute_with_observation(), extract_trace_context(), get_metrics_buffer(), get_spans_buffer(), get_trace_context() (+20 more)

### Community 37 - "Event Store Save Tests"
Cohesion: 0.14
Nodes (15): FileChunk, FileChunk, Port for semantic search over uploaded file chunks., _cosine_similarity(), PrismFileSearch, FileSearchPort implementation using Neuro embeddings and uploaded file sidecars., Searches uploaded file chunks by semantic similarity., Lazy-load Neuro embedder to avoid circular imports at module load. (+7 more)
...
Cohesion: 0.10
Nodes (17): ContextAnalysisRequest, GenerateImageRequest, Request model for image generation., Request model for running pipeline with external context., Sanitize text for LLM prompts without HTML escaping.      Reuses InputSanitizer, sanitize_for_prompt(), GenerateImageRequest, Return name unchanged if valid, else raise ValueError. (+9 more)
...
Cohesion: 0.12
Nodes (23): ClearHistoryCommand, ClearMemoryCommand, ClearPipelineCacheCommand, Command, DeleteHistoryCommand, ExecuteWidgetCommand, Application Layer - Commands (Write Operations)  Commands represent intentions t, Command to store conversation in memory. (+15 more)

### Community 43 - "Community 43"
Cohesion: 0.13
Nodes (23): is_retryable(), ProviderTimeoutError, ProviderUnavailableError, Provider request timed out.          This error IS retryable - the provider ma, Provider service is unavailable.          This error IS retryable - may be a t, Check if an error is retryable.      Args:         error: Exception to check, Exception, CircuitOpenError (+15 more)

### Community 44 - "Community 44"
Cohesion: 0.11
Nodes (25): AttachmentRef, CalculationRequest, DiscoverRequest, Pydantic request/response schemas for the Reasoner API., StockRequest, SuggestionRequestModel, WeatherRequest, BaseModel (+17 more)

### Community 45 - "Community 45"
Cohesion: 0.18
Nodes (23): _get_v(), _is_debate(), _is_orchestrated(), _is_scientific(), _is_socratic(), Deep Read serializer., Serializer dedicated to the article Pre-Mortem phase (4.25)., Serializer dedicated to the article Journal Review phase (4.5). (+15 more)

### Community 46 - "Community 46"
Cohesion: 0.05
Nodes (46): build_auto_preset(), get_method_from_preset(), get_preset_price_tier(), get_preset_tier(), PipelinePreset, Core preset domain logic: dataclass, validation, and helpers., Extract method name from preset string., Infer pricing tier from preset ID suffix. (+38 more)
...
Cohesion: 0.16
Nodes (21): _is_circuit_open(), Cross-encoder reranking service for search and memory retrieval.  Supports Coh, Score a single document against a query using Nemotron Rerank VL via logprobs., Rerank documents using NVIDIA Nemotron Rerank VL (free, via OpenRouter)., Rerank memory chunks (ContextChunk objects) before compression.      Args:, Strip null bytes, normalize Unicode, truncate., Lightweight circuit breaker based on recent failures., Rerank documents via cross-encoder (Cohere via OpenRouter by default).      Ar (+13 more)

### Community 49 - "Community 49"
Cohesion: 0.05
Nodes (72): _chunk_text(), _cosine_similarity(), DocumentVectorStore, Per-session semantic vector store for uploaded documents.  Chunks uploaded file, Chunk and embed a document, writing embeddings to a JSON sidecar.          Args:, Retrieve the top-k most relevant chunks for a query.          Args:, Delete a document's vector sidecar., Split text into overlapping chunks.      Args:         text: Source text. (+64 more)

### Community 50 - "Community 50"
Cohesion: 0.07
Nodes (28): 10e1a925-9d05-4e60-862b-aab21cbd2456, 1b020eb7-4eda-42f5-9908-18d5fc520965, 225c9182-38f2-46b5-bdbd-be7413f60905, 2369eb54-5ba4-423c-8b54-934a79e4c970, 33e7fce5-28ca-4ace-a0e7-f0999acd7930, 5dfd82eb-9bae-4454-a7f0-ddb5c453992c, 5e84f31a-8fd4-47d1-9eaf-801b91850130, 6b3cfaa4-2180-4a77-8591-361804a89a6a (+20 more)
...
Cohesion: 0.04
Nodes (45): ChatErrorBoundary, ChatErrorBoundaryProps, ChatErrorBoundaryState, ChatPage(), MessagesAction, PipelineSkeleton, PipelineSkeletonProps, useKeyboardShortcuts() (+37 more)
...
Cohesion: 0.15
Nodes (13): execute_phases_dag(), PhaseFn, PhaseStep, PipelineFlow, Phase sequence registry and dispatcher for reasoning methods., A single step in a pipeline phase sequence., Registry that maps method names to ordered phase sequences., Register a phase sequence for a reasoning method. (+5 more)
...
Cohesion: 0.06
Nodes (42): How temperature should adapt on retry., TemperatureStrategy, EventPublisher, LLMGenerationCompleted, LLM generation completed for a specific phase/role., EventPublisher, Core port: EventPublisher — decouples infrastructure from application event bus., Minimal publish interface satisfied by EventBus and any compatible adapter. (+34 more)

### Community 58 - "Community 58"
Cohesion: 0.07
Nodes (38): get_phase_retry_budget(), get_phase_timeout(), get_quality_judge_model(), get_quality_judge_threshold(), get_token_budget(), Single source of truth for all hardcoded constants used across the Reasoner proj, Get token budget for a specific role/phase., Get token budget for a specific role/phase. (+30 more)

### Community 59 - "Community 59"
Cohesion: 0.09
Nodes (30): EncryptionService, PostgreSQLEventStore, test_encryption(), Record, _fetch_legacy_events(), _fetch_legacy_snapshots(), main(), migrate_events() (+22 more)
...
Cohesion: 0.09
Nodes (32): api_models(), api_presets(), detect_widgets(), execute_widget(), get_suggestions(), list_widgets(), Widget, suggestions, and UI status endpoints., List all available widgets. (+24 more)
...
Cohesion: 0.15
Nodes (18): generate_csrf_token(), generate_signed_csrf_token(), _get_csrf_secret(), Backend CSRF token generation and validation.  Implements a stateless Double-Sub, Generate and sign a fresh CSRF token., Get the CSRF signing secret. CSRF_SECRET must be set independently of ADMIN_API_, Generate a token that embeds a signed expiry timestamp., Sign a token with HMAC-SHA256. Returns 'token.signature_hex'. (+10 more)

### Community 65 - "Community 65"
Cohesion: 0.09
Nodes (28): inter, AuthProvider(), Providers(), useConversationHistory(), Hero(), NeuroPanel(), Sidebar, SidebarComponent() (+20 more)

### Community 66 - "Community 66"
Cohesion: 0.05
Nodes (53): Execute the reasoning pipeline., Decorator that records phase duration in ``state.phase_durations``., Refactored Reasoner Orchestrator.     Now uses WorkflowStrategy composition ins, ReasonerPipeline, timed(), make_event(), Factory function to create domain events.          Automatically sets:     - eve, ExecuteWidgetCommand (+45 more)

### Community 68 - "Community 68"
Cohesion: 0.06
Nodes (36): ABC, ProviderConfig, ResilientProviderConfig, AnthropicReasoning, CircuitBreaker, _create_embedding(), _create_reasoning(), create_resilient_embedding() (+28 more)

### Community 69 - "Community 69"
Cohesion: 0.19
Nodes (17): enrich_problem_with_youtube(), _enrich_query(), _extract_youtube_id(), _fetch_youtube_metadata(), Search and vetting phase logic., run_context_vetting_phase(), run_deep_read_phase(), validate_evidence_coverage() (+9 more)
...
Cohesion: 0.09
Nodes (22): Any, float, Match, Widget Protocol (Hexagonal Architecture Port)  Defines the interface that all wi, Execute the widget with given parameters.                  Args:             par, Extract parameters from the query.                  Args:             query: Use, Extract parameters from query using regex patterns., Types of widgets available in Reasoner. (+14 more)

### Community 72 - "Community 72"
Cohesion: 0.10
Nodes (27): build_vs_prompt(), compute_verbalized_entropy(), _extract_json_block(), parse_vs_response(), Verbalized Sampling primitives — generation, parsing, normalization, sampling., Strip fences, regex-extract JSON, validate structure., Probability-weighted sample., Deterministic; tie → first. (+19 more)
...
Cohesion: 0.16
Nodes (9): Centralized environment-aware settings.  This is the ONLY module in the projec, Base URL for internal self-calls (e.g., Neuro endpoints from streaming)., Parse NEURO_REASONING_FALLBACK_MODELS into a list., Parse NEURO_EMBEDDING_FALLBACK_MODELS into a list., Parse CORS_ORIGINS env var into a list of origin strings., Parse CORS_ORIGINS env var into a list of origin strings., Application settings derived from environment variables., Settings (+1 more)

### Community 76 - "Community 76"
Cohesion: 0.06
Nodes (46): APIRouter, Lock, NeuroConfig, PersonaConfig, AuditRequest, AuditResponse, build_audit_system_prompt(), _cached_compress() (+38 more)
...
Cohesion: 0.13
Nodes (47): DELETE(), POST(), POST(), POST(), POST(), POST(), POST(), API (+39 more)

### Community 79 - "Community 79"
Cohesion: 0.08
Nodes (29): get_auth_adapter(), Auth adapter factory.  Selects the appropriate auth adapter based on environment, Get or create the global auth adapter., Override the auth adapter (useful for tests)., set_auth_adapter(), LocalAuthAdapter, Local Auth Adapter — Implements AuthPort using local JWT signing.  Used for: - D, Development auth adapter using local HS256 JWT.      Generates and validates tok (+21 more)

### Community 80 - "Community 80"
Cohesion: 0.12
Nodes (39): benchmark_vs_latency(), main(), measure_pipeline(), _MockLLM, _MockNLI, bool, float, str (+31 more)

### Community 81 - "Community 81"
Cohesion: 0.05
Nodes (50): RateLimitError, Rate limit exceeded.          This error IS retryable after the specified dela, AuthenticationError, BaseLLMProvider, DegradedLLMResponse, is_retryable(), LLMConfig, LLMError (+42 more)

### Community 82 - "Community 82"
Cohesion: 0.07
Nodes (28): GET(), Topic, websitesForTopic, ImageSearchBody, searchSearxng(), SearxngSearchOptions, SearxngSearchResult, ChatTurnMessage (+20 more)

### Community 83 - "Community 83"
Cohesion: 0.14
Nodes (19): _bm25_score(), _get_searxng_cb(), _normalize_url(), _parse_freshness(), Reasoner - Web Discovery Tool Provides internal web search capabilities for con, Normalize URL for deduplication.      Handles protocol, www, trailing slashes (, Simplified BM25-style relevance score between a query and a search result., Derive a freshness score in [0, 1] from SearXNG's publishedDate field.      Re (+11 more)
...
Cohesion: 0.15
Nodes (22): ClaimExtractionMode, _extract_claims(), extract_claims_from_vs_candidates(), ExtractedClaimSet, _LLMClient, VS Claim Extraction stage — ClaimExtractionStage., Extract factual claims from text using LLM, with sentence-split fallback., VSClaimExtractionConfig (+14 more)
...
Cohesion: 0.24
Nodes (11): analyze_results(), check_searxng_health(), main(), bool, int, str, Query scientific method and verify relevant academic sources are still found., Analyze search results and return a quality report. (+3 more)
...
Cohesion: 0.19
Nodes (10): ErrorEntry, ErrorStats, Error Persistence Layer  Stores application errors in SQLite for durability and, Single error occurrence with full context., Aggregated error statistics., ClientErrorReport, Error reporting and admin error log endpoints.  POST /api/error-report  — client, Accept error reports from the frontend. No auth required. (+2 more)
...
Cohesion: 0.05
Nodes (41): APIKey, AuthenticationError, AuthManager, AuthorizationError, get_auth_manager(), Simple Token-Based Authentication Middleware Production-ready auth with API key, Hash API key using SHA-256., Set a key with LRU eviction. (+33 more)

### Community 277 - "Community 277"
Cohesion: 0.05
Nodes (40): asdict(), CircuitBreaker, CircuitBreakerConfig, CircuitBreakerStats, CircuitState, get_all_circuit_breakers(), get_circuit_breaker(), _get_memory_circuit_breaker() (+32 more)

### Community 278 - "Community 278"
Cohesion: 0.07
Nodes (40): MemoryEventType, Widget lifecycle events., WidgetEventType, EventStore, get_event_store(), Event Persistence Layer  Stores domain events in SQLite for durability and repla, Save events to the event store.          Events are persisted atomically within, Determine aggregate type from event type. (+32 more)
...
Cohesion: 0.05
Nodes (43): BiasCritiqueSubAgent, BiasCritiqueSubAgent — ONE JOB: detect cognitive biases and framing effects., CounterfactualSubAgent, CounterfactualSubAgent — ONE JOB: explore "what if the opposite were true?" for, EvidenceCritiqueSubAgent, EvidenceCritiqueSubAgent — ONE JOB: evaluate quality and reliability of sources/, CritiqueHyperAgent, CritiqueHyperAgent — orchestrates 4 parallel critique subagents and synthesizes (+35 more)

### Community 317 - "Community 317"
Cohesion: 0.22
Nodes (9): do_run_migrations(), Alembic environment configuration., Run migrations in 'offline' mode., Run migrations in 'online' mode with async engine., Run migrations in 'online' mode., run_async_migrations(), run_migrations_offline(), run_migrations_online() (+1 more)
...
Cohesion: 0.29
Nodes (9): _kill_ports(), _kill_processes(), _kill_processes_fallback(), main(), Fallback when psutil is missing., Force-free known ports (handles Windows zombie sockets)., Stop the SearXNG container via docker compose., Find and terminate/kill Reasoner-related processes. Returns count. (+1 more)

### Community 321 - "Community 321"
Cohesion: 0.07
Nodes (33): FeedbackEntry, FeedbackStats, FeedbackStore, Feedback Persistence Layer  Stores user feedback in SQLite for durability and qu, Initialize database schema., Backfill existing JSONL data if table is empty and JSONL exists., Synchronous insert. Must be called inside lock., Synchronous stats aggregation. Must be called inside lock. (+25 more)

### Community 322 - "Community 322"
Cohesion: 0.07
Nodes (35): _fire_and_forget(), get_postgres_store(), initialize_postgres_store(), PostgreSQLEventStore, PostgreSQL Event Store  Production-grade event persistence using PostgreSQL. Sup, Initialize database schema., Save events atomically.                  Raises:             asyncpg.Error: If d, Internal method for saving events, allowing external decorators to wrap it. (+27 more)

### Community 323 - "Community 323"
Cohesion: 0.06
Nodes (22): CAPABILITIES, Logo(), LogoProps, faqs, COLOR_TEXT, COLOR_TEXT_MUTED, COLOR_TEXT_SUBTLE, generateNodes() (+14 more)
...
Cohesion: 0.07
Nodes (37): _get_pipeline_owner(), Pipeline ownership tracking — shared between API and websocket layers.  Extrac, Return the user_id that owns *pipeline_id*, or None if not tracked., Persist ownership mapping for a pipeline run., _save_pipeline_owner(), get_websocket_stats(), Get WebSocket connection statistics., str (+29 more)

### Community 327 - "Community 327"
Cohesion: 0.07
Nodes (34): DashboardContent(), HistoryEntry, QuotaStatus, useQuota(), SubscriptionStatus, useSubscription(), NeuroPanelProps, RecallChunk (+26 more)

### Community 328 - "Community 328"
Cohesion: 0.05
Nodes (37): BaseSubAgent, BaseSubAgent — abstract base class every HyperGate sub-agent must extend.  Eac, Extract the first valid JSON object from raw LLM text., One job. One cache. One execute() call., Return the focused system prompt for this sub-agent's single task., Parse the LLM's raw text into a structured dict.         Must always return a d, Run the sub-agent; returns SubAgentOutput (never raises)., Any (+29 more)

### Community 329 - "Community 329"
Cohesion: 0.32
Nodes (6): check_gptzero(), process_humanization_task(), apply_zws_cloak(), pii_masking(), str, Zero-Width Space Injection Algorithm.
...
Cohesion: 0.25
Nodes (7): get_api_keys_status(), API key status and validation endpoints., Get status of all configured LLM provider API keys.      Returns which keys ar, Pre-flight validation of API keys.      Tests each configured provider with a, validate_api_keys(), Request, User
...
Cohesion: 0.07
Nodes (32): Public endpoint for Stripe webhooks., stripe_webhook(), get_event_store(), FastAPI dependency: provides the shared EventStore., lifespan(), Background task to update active users gauge every 60s (Critical Enhancement 7.3, Application lifespan: startup and shutdown orchestration., _update_active_users_loop() (+24 more)
...
Cohesion: 0.33
Nodes (4): metrics_endpoint(), Prometheus metrics endpoints (FastAPI layer).  Metric definitions are in reaso, Expose Prometheus metrics.      Critical Enhancement 7.2: generate_latest is s, Response

### Community 342 - "Community 342"
Cohesion: 0.06
Nodes (38): EventStoreCorruptionError, Raised when event store data is inconsistent (e.g. version gaps after compaction, __getattr__(), Infrastructure Persistence Package  Event storage and retrieval for event-source, Lazy-load PostgreSQL symbols so asyncpg is not required at import time., Snapshot Strategy for Aggregate Performance  Periodic snapshots allow faster agg, Serialize aggregate state for storage., Load snapshot from storage. (+30 more)
...
Cohesion: 0.08
Nodes (23): Any, PipelineState, str, Any, PipelineState, str, FinalSolution, PipelineState (+15 more)
...
Cohesion: 0.07
Nodes (26): BackgroundBlobs(), BackgroundBlobsProps, useMounted(), usePrefersReducedMotion(), copyToClipboard(), esc(), formatMarketCap(), Badge() (+18 more)
...
Cohesion: 0.50
Nodes (3): PerspectiveDefinition, Data-driven perspective definitions for Phase 2 (Multi-Perspective Analysis).  R, Self-contained definition of one analysis perspective.      name        — matche

### Community 360 - "Community 360"
Cohesion: 0.05
Nodes (42): ═══════════════════════════════════════════════════════════════════════════════, 1. models.py (2 functions), 2. renderer.py (1 function), ═══════════════════════════════════════════════════════════════════════════════, 3. infrastructure/persistence/event_store.py (10 functions), 4. infrastructure/persistence/postgres_store.py (4 functions), 5. infrastructure/widgets/registry.py (1 function), 6. neuro/config.py (1 function) (+34 more)
...
Cohesion: 0.40
Nodes (4): configurations, configurations, version, version
...
Cohesion: 0.11
Nodes (23): _get_build_provider(), TestSearchFallbackBug, _decompose_query(), DiscoveryClient, _extract_search_keywords(), get_search_client(), get_searxng_base_url(), PerplexitySearchClient (+15 more)
...
Cohesion: 0.12
Nodes (20): CoverageValidatorSubAgent, CoverageValidatorSubAgent — ONE JOB: verify that sub-problems cover the full ori, DecompositionHyperAgent, DecompositionHyperAgent — orchestrates 3 parallel decomposition subagents.  Ph, Orchestrates parallel decomposition subagents., StakeholderMapperSubAgent — ONE JOB: identify relevant perspectives and stakehol, StakeholderMapperSubAgent, StructuralDecomposerSubAgent — ONE JOB: hierarchically decompose the problem int (+12 more)
...
Cohesion: 0.11
Nodes (14): AuthError, getAuthToken(), getCurrentUser(), getEnabledOAuthProviders(), getSession(), guardSupabase(), signInWithEmail(), signInWithOAuth() (+6 more)

### Community 477 - "Community 477"
Cohesion: 0.09
Nodes (22): GapIdentifierSubAgent, GapIdentifierSubAgent — ONE JOB: identify what evidence is still missing after s, SearchHyperAgent — orchestrates 3 parallel search subagents.  Phase 1 (paralle, Orchestrates parallel search subagents., SearchHyperAgent, QueryGeneratorSubAgent, QueryGeneratorSubAgent — ONE JOB: generate diverse search queries for web resear, SourceEvaluatorSubAgent — ONE JOB: evaluate credibility and relevance of search (+14 more)
...
Cohesion: 0.10
Nodes (23): BaseException, GateDecision, HyperContext, _extract_json(), GateAgent, GateDecision, Gate Agent — Lightweight pre-flight router that decides whether a user prompt s, Return a GateDecision. Any failure falls back to pipeline. (+15 more)

### Community 483 - "Community 483"
Cohesion: 0.11
Nodes (32): calculate_expression(), geocode_location(), get_discover_content(), get_stock_data(), get_weather_data(), _has_yahooquery(), _has_yfinance(), Widgets Backend Engine Provides data for smart widgets: Weather, Stocks, Calcula (+24 more)

### Community 484 - "Community 484"
Cohesion: 0.10
Nodes (23): ErrorStore, Run sync function in thread pool with locking., Initialize database schema., Coerce to int within bounds — prevents SQL injection via .format()., Remove errors older than retention period., Insert an error entry asynchronously., Query errors with optional filters., Synchronous stats aggregation. (+15 more)

### Community 485 - "Community 485"
Cohesion: 0.12
Nodes (30): _docker_available(), _import_smoke_test(), main(), _port_in_use(), print_banner(), Attempt to free a port. Returns True if freed., Poll the backend root endpoint until it responds (any 2xx–4xx means it's up)., Start a subprocess and return the handle. (+22 more)
...
Cohesion: 0.16
Nodes (14): CacheConfig, ContextChunk, cosine_similarity(), L1Cache, L2Index, l3_scan(), Neuro Cache Hierarchy L1/L2/L3 with persona-aware similarity thresholds., Scan L3 (cold) memory files for relevant context.      Embeddings are cached as (+6 more)
...
Cohesion: 0.10
Nodes (16): Per-run cancellation state management for the Reasoner API.  Encapsulates the, Check if a run is currently active (non-locking, best-effort)., Return the user_id that owns a run, or None if anonymous/not found., Return a snapshot of active run IDs., Clear all state (for test isolation)., Thread-safe / asyncio-safe store for per-run cancellation state.      Encapsul, Register a new run and return its cancel event., Evict entries older than MAX_STALE_SECONDS or beyond MAX_ENTRIES. (+8 more)

### Community 494 - "Community 494"
Cohesion: 0.12
Nodes (28): _extract_user_id(), generic_exception_handler(), _get_error_store(), http_exception_handler(), _log_error(), Global Exception Handlers for FastAPI  Captures all unhandled exceptions, HTTP e, Build a safe JSON error response that doesn't leak internals., Handle HTTPException (4xx, 5xx from FastAPI/Starlette). (+20 more)
...
Cohesion: 0.08
Nodes (23): 10. Living Documentation, 1. Project Overview, 2. Technology Stack, 3. Project Structure, 4. Commands, 5. Key Architecture Details, 6. Working with Neuro & Compression, 7. Cross-Cutting Concerns (+15 more)
...
Cohesion: 0.08
Nodes (33): check_rate_limit(), _get_auth_manager_instance_auth_deps(), get_client_id(), _get_rate_limiter_instance_auth_deps(), optional_auth(), Authentication and rate-limiting FastAPI dependencies., Require valid API key authentication.     Raises HTTPException if authentication, Optional authentication - returns API key if provided, None otherwise. (+25 more)

### Community 507 - "Community 507"
Cohesion: 0.08
Nodes (24): Architecture, CLI, Commits, Conventions, Dev (start everything), Entry points, Frontend, Gotchas (+16 more)

### Community 508 - "Community 508"
Cohesion: 0.14
Nodes (13): Any, float, int, str, WidgetDetectionResult, WidgetResult, Calculate confidence score for widget detection.                  Factors:, Execute a widget by name.          Args:             widget_name: Name of wid (+5 more)
...
Cohesion: 0.16
Nodes (20): article_draft_prompt(), article_refine_prompt(), article_retrieval_plan_prompt(), article_verify_prompt(), build_followup_context(), build_synthesis_context(), detect_language(), _followup_context() (+12 more)

### Community 514 - "Community 514"
Cohesion: 0.24
Nodes (21): Wrap user-controlled text in explicit delimiters., _wrap_user_input(), classification_prompt(), cot_detection_batch_prompt(), cot_detection_prompt(), cross_verification_prompt(), decomposition_prompt(), deep_read_prompt() (+13 more)
...
Cohesion: 0.13
Nodes (22): _compute_image_hash(), describe_image(), _detect_mime_type(), _encode_image_base64(), ocr_image(), Image and document extraction utilities using vision-capable LLMs., Extract verbatim text from an image using an OCR-optimized model via OpenRouter., Compute a simple hash for image deduplication. (+14 more)

### Community 517 - "Community 517"
Cohesion: 0.16
Nodes (11): Get the cancel event for a run., Signal cancellation for a run., Check if a run is currently active (non-locking, best-effort)., Return the user_id that owns a run, or None if anonymous/not found., Return a snapshot of active run IDs., Check if a run has been requested to cancel., Atomically check and clear cancellation flag (9.2: Lua script)., Lazy-create the in-memory fallback store. (+3 more)
...
Cohesion: 0.27
Nodes (9): Any, bytes, int, str, _check_depth(), Safe JSON loading with depth limits to prevent stack exhaustion., Recursively check nesting depth of parsed JSON object., Parse JSON with a strict depth limit.      Args:         data: JSON string or by (+1 more)

### Community 522 - "Community 522"
Cohesion: 0.14
Nodes (19): AuthenticationError, InfrastructureError, is_retryable(), LLMError, MemoryError, ModelNotFoundError, ProviderTimeoutError, ProviderUnavailableError (+11 more)
...
Cohesion: 0.14
Nodes (10): Return True if the state store is shared across all workers (Redis up)., Clean up a run's state., Cancel all active runs in Redis using SMEMBERS (O(1) per member, not SCAN)., Clear all state (for test isolation)., Remove a run from active set., Manages pipeline run state in Redis with in-memory fallback., Lazy-load Redis client., Load the atomic pop-cancelled Lua script once. (+2 more)
...
Cohesion: 0.13
Nodes (13): Any, AsyncClient, bool, str, DeepLClient, _get_base_url(), DeepL API client for text translation., Quick health check by querying usage. (+5 more)
...
Cohesion: 0.31
Nodes (17): get_language_instruction(), academic_sot_skeleton_prompt(), academic_sot_solve_prompt(), article_cove_answer_prompt(), article_cove_draft_prompt(), article_cove_revise_prompt(), article_cove_verify_prompt(), article_pre_mortem_prompt() (+9 more)
...
Cohesion: 0.09
Nodes (28): AuthenticationError, ConfigurationError, JSONExtractionError, JSONValidationError, ModelNotFoundError, ParseError, PhaseError, PipelineError (+20 more)
...
Cohesion: 0.22
Nodes (8): InMemoryVSEntropyStore, log_vs_behavioral_audit(), VS Behavioral Audit Observability stage., VSEntropyStore, float, int, VSFeatureFlags, VSGenerationResult
...
Cohesion: 0.17
Nodes (11): Any, Match, str, CalculatorWidget, Calculator Widget  Evaluates mathematical expressions using simpleeval (safe eva, Format result for display., Calculator widget for mathematical expressions.      Features:     - Basic arith, Extract expression from match. (+3 more)
...
Cohesion: 0.19
Nodes (14): Verbalized Sampling brainstorming phase prompts and prompt builders.  VS (Verb, Build the clustering + scoring prompt from the raw idea pool., Build the final synthesis prompt from developed ideas and cluster themes., Build the deep-development prompt for the selected top ideas., Build the VS idea-generation prompt for one round., vs_cluster_prompt(), vs_develop_prompt(), vs_generation_prompt() (+6 more)
...
Cohesion: 0.23
Nodes (10): GET(), generateSignedCsrfToken(), getCsrfSecret(), signCsrfToken(), verifyCsrfToken(), REASONER_WS_HOSTS, REASONER_WS_PORTS, buildConnectSrc() (+2 more)
...
Cohesion: 0.16
Nodes (10): HyperContext, Data models for the HyperGate sub-agent communication protocol.  All models ar, Sent by HyperGateAgent to each sub-agent., Returned by every sub-agent back to HyperGateAgent., Assembled by HyperGateAgent after Phase 1 completes.     Passed (as to_dict()), Serialise for injection into TieBreakerSubAgent context., SubAgentInput, SubAgentOutput (+2 more)
...
Cohesion: 0.33
Nodes (7): CompressionLevel, ContextCompressor, Language, Neuro Compression Module Optimizes text context for LLM efficiency. Uses "Neur, Convenience helper for quick compression., smart_compress(), str

### Community 561 - "Community 561"
Cohesion: 0.25
Nodes (12): audit_claim_coverage_vs(), _check_overlap_with_evidence(), CoverageAuditResult, GapType, _generate_paraphrases_vs(), _LLMClient, VS Coverage Audit stage — CoverageAuditStage., Generate paraphrases of a claim using VS. (+4 more)
...
Cohesion: 0.21
Nodes (13): _cache_key(), get_cache_stats(), _load_cache(), _prune_disk_cache(), _prune_memory_cache(), Remove oldest cache files if the directory exceeds the max size., # NOTE: This is a per-process cache. For horizontal scaling, use Redis or, Return memory cache telemetry. (+5 more)
...
Cohesion: 0.22
Nodes (9): Any, Match, str, DiscoverWidget, Discover Widget  Provides trending content aggregation by topic., Search using SearXNG., Discover widget for trending content.          Features:     - Topic-based aggre, Extract topic from match. (+1 more)

### Community 577 - "Community 577"
Cohesion: 0.22
Nodes (9): Any, int, Match, str, ImageSearchWidget, Image Search Widget  Searches for images using SearXNG., Image search widget using SearXNG.          Features:     - Visual search result, Extract search query from match. (+1 more)

### Community 578 - "Community 578"
Cohesion: 0.22
Nodes (9): Any, int, Match, str, Video Search Widget  Searches for videos using SearXNG., Video search widget using SearXNG.          Features:     - Video search results, Extract search query from match., Search videos using SearXNG. (+1 more)

### Community 579 - "Community 579"
Cohesion: 0.17
Nodes (11): 0. Executive Summary, 14. Known Architecture Violations, 22. Future Architecture Roadmap, 23. References & File Locations, 2.1 Backend, 2.2 Frontend, 2. Technology Stack, 6.1 Method Registration (+3 more)
...
Cohesion: 0.27
Nodes (10): generate_probes_with_vs(), _LLMClient, ProbeGenerationConfig, ProbeSet, VS Probe Generation stage — IntentConsistencyStage., Simplified semantic distance using sequence matcher (0.0=identical, 1.0=complete, _semantic_distance(), float (+2 more)
...
Cohesion: 0.33
Nodes (10): _extract_style_brief(), Article writing pipeline phase logic., Extract author and publication hints from the user's problem statement., run_article_adversarial_verify_phase(), run_article_draft_phase(), run_article_refine_phase(), run_article_retrieve_sources_phase(), PipelineState (+2 more)
...
Cohesion: 0.22
Nodes (10): _patch_openai_platform_detection(), _perplexity_response_format(), Low-level LLM utilities: platform patches, JSON heuristics, response formatting., Windows WMI can hang indefinitely when the openai library calls     platform.sys, Heuristic: only enable structured outputs for prompts that already demand pure J, Return a permissive JSON-schema response format for compatible Perplexity models, _requests_strict_json(), Any (+2 more)

### Community 2211 - "Community 2211"
Cohesion: 0.35
Nodes (10): cove_answer_prompt(), cove_answer_prompt_single(), cove_crosscheck_prompt(), cove_draft_prompt(), cove_revise_prompt(), cove_verify_prompt(), Factored variant (CoVE paper Sec 3.3). One question per call, no baseline draft., Generate a cross-check prompt to detect inconsistencies between     independent (+2 more)

### Community 2212 - "Community 2212"
Cohesion: 0.35
Nodes (10): color(), get_current_branch(), is_runtime_artifact(), main(), parse_status(), Remove stale git index.lock if present., Check if a file path matches runtime artifact patterns., Parse git status --porcelain and split into source vs runtime files. (+2 more)
...
Cohesion: 0.13
Nodes (24): main(), Neuro CLI Manage the internal memory engine., Neuro Memory Engine CLI., Return value if numeric and not NaN, else '?'., Check health and statistics., Start the memory engine server., _safe_num(), start() (+16 more)
...
Cohesion: 0.20
Nodes (5): Register a new run and return its cancel event., Signal cancellation for all active runs., Mark a run as active (idempotent)., Cancel all currently active runs. Returns count., int
...
Cohesion: 0.44
Nodes (8): coding_assemble_prompt(), coding_generate_prompt(), coding_review_prompt(), coding_spec_prompt(), coding_tests_prompt(), Coding pipeline prompts — production-grade code generation., PipelineState, str

### Community 2253 - "Community 2253"
Cohesion: 0.25
Nodes (6): FineTunedProvider, Fine-Tuned Model Provider  Adapter for custom fine-tuned models hosted on OpenAI, Provider for fine-tuned models on custom endpoints.      Inherits all retry, str, Any, int, str
...
Cohesion: 0.22
Nodes (7): Any, str, PhaseSubAgentInput, PhaseSubAgentOutput, Data models for PhaseSubAgent communication protocol.  Phase-level subagents rec, Sent by a PhaseHyperAgent to each sub-agent., Returned by every phase sub-agent back to its hyper-agent.
...
Cohesion: 0.29
Nodes (7): checkConstraints, compositePrimaryKeys, foreignKeys, indexes, name, uniqueConstraints, chats
...
Cohesion: 0.39
Nodes (7): delphi_aggregation_prompt(), delphi_convergence_prompt(), delphi_dissent_prompt(), delphi_round1_prompt(), delphi_round2_prompt(), int, str

### Community 2268 - "Community 2268"
Cohesion: 0.25
Nodes (5): Distributed run state backed by Redis with in-memory fallback.  Replaces modul, Internal signal that Redis is not reachable., Circuit-breaker style: don't hammer Redis if it's down., Execute a Redis coroutine factory, falling back to memory on failure., _RedisUnavailable

### Community 2269 - "Community 2269"
Cohesion: 0.14
Nodes (13): Widget, get_widget_registry(), Widget Registry  Central registry for all available widgets. Handles discover, Central registry for all widgets.          Provides:     - Widget registratio, Clear execution statistics., Get or create the global widget registry., Reset the global registry (for testing)., Register default widgets. (+5 more)
...
Cohesion: 0.25
Nodes (8): checkConstraints, compositePrimaryKeys, foreignKeys, indexes, name, uniqueConstraints, tables, messages
...
Cohesion: 0.57
Nodes (6): analogical_abstraction_prompt(), analogical_domain_search_prompt(), analogical_mapping_prompt(), analogical_transfer_prompt(), PipelineState, str

### Community 2296 - "Community 2296"
Cohesion: 0.57
Nodes (6): bayesian_likelihood_prompt(), bayesian_posterior_prompt(), bayesian_prior_prompt(), bayesian_sensitivity_prompt(), PipelineState, str

### Community 2297 - "Community 2297"
Cohesion: 0.57
Nodes (6): debate_cross_examine_prompt(), debate_judge_prompt(), debate_opening_prompt(), debate_rebuttal_prompt(), PipelineState, str

### Community 2298 - "Community 2298"
Cohesion: 0.57
Nodes (6): dialectical_antithesis_prompt(), dialectical_aufhebung_prompt(), dialectical_contradictions_prompt(), dialectical_thesis_prompt(), PipelineState, str

### Community 2299 - "Community 2299"
Cohesion: 0.57
Nodes (6): jury_critic_prompt(), jury_generator_prompt(), jury_meta_eval_prompt(), jury_verifier_prompt(), PipelineState, str

### Community 2300 - "Community 2300"
Cohesion: 0.57
Nodes (6): pre_mortem_backtrack_prompt(), pre_mortem_failure_prompt(), pre_mortem_redesign_prompt(), pre_mortem_signals_prompt(), PipelineState, str

### Community 2301 - "Community 2301"
Cohesion: 0.57
Nodes (6): tot_backtrack_prompt(), tot_decompose_prompt(), tot_evaluate_prompt(), tot_generate_prompt(), PipelineState, str
...
Cohesion: 0.60
Nodes (5): critique_prompt(), perspective_prompt(), stress_test_prompt(), PipelineState, str

### Community 2334 - "Community 2334"
Cohesion: 0.60
Nodes (5): pot_execute_prompt(), pot_generate_prompt(), pot_interpret_prompt(), PipelineState, str

### Community 2335 - "Community 2335"
Cohesion: 0.60
Nodes (5): sd_adapt_prompt(), sd_implement_prompt(), sd_select_prompt(), PipelineState, str
...
Cohesion: 0.22
Nodes (4): Audit Service — Logs query executions as domain events.  Publishes to the Even, Auth Service — Thin wrapper over AuthPort with caching and logging., Billing Service — Orchestrates checkout, portal, and webhook sync., Quota Service — Application-layer orchestrator for usage limits.  Enforces busin
...
Cohesion: 0.40
Nodes (4): deep_research_prompt(), int, PipelineState, str

### Community 2359 - "Community 2359"
Cohesion: 0.60
Nodes (4): scientific_hypothesis_prompt(), scientific_test_prompt(), PipelineState, str

### Community 2360 - "Community 2360"
Cohesion: 0.60
Nodes (4): socratic_answer_prompt(), socratic_question_prompt(), PipelineState, str
...
Cohesion: 0.40
Nodes (5): 16.1 Token Optimization Flags, 16.2 Cascading Model Fallback (Cost Optimization), 16.3 Parallel Execution, 16.4 Preset Cost Estimates, 16. Performance & Optimization
...
Cohesion: 0.33
Nodes (5): get_client_ip(), Trusted-proxy-aware client IP resolution.  Prevents X-Forwarded-For spoofing by, Resolve the actual client IP from a request.      Strategy:     1. If no trusted, Request, str
...
Cohesion: 0.50
Nodes (4): 18.1 Multi-Tenant SaaS, 18.2 Billing Integration, 18.3 Environment-Specific Configuration, 18. Deployment & SaaS Features

### Community 2389 - "Community 2389"
Cohesion: 0.50
Nodes (4): 19.1 Test Organization, 19.2 Coverage Targets, 19.3 Auto-Generated Tests, 19. Testing Strategy
...
Cohesion: 0.67
Nodes (3): 20.1 Conventional Commits, 20.2 CI/CD Pipeline, 20. Git Workflow & CI/CD

### Community 2412 - "Community 2412"
Cohesion: 0.67
Nodes (3): 5.1 Phase Configuration, 5.2 Token Budgets, 5. Core Pipeline Flow (6 Phases)

### Community 2483 - "Community 2483"
Cohesion: 0.40
Nodes (5): VS Two-Tier Verification Routing stage., route_claim_by_vs_probability(), VerificationRoute, float, VSFeatureFlags
...
## Knowledge Gaps
- **21025 isolated node(s):** `protocol_version`, `timestamp`, `mode`, `risk_tolerance`, `context_strategy` (+21020 more)
...
- **Why does `PipelineState` connect `Article Pipeline` to `Authentication & Security`, `Event Sourcing & Domain Events`, `Verbalized Sampling & Benchmarks`, `Community 513`, `Widgets & Calculator`, `Community 514`, `Image Generation API`, `Community 533`, `Chat UI Components`, `JSON Parsing Utilities`, `Community 2333`, `Community 2334`, `Community 2335`, `Event Store Count Tests`, `Community 2336`, `Community 2211`, `Community 552`, `Community 45`, `Community 47`, `Community 2358`, `Community 55`, `Community 2359`, `Community 57`, `Community 58`, `Community 2360`, `Community 316`, `Community 2238`, `Community 66`, `Community 69`, `Community 977`, `Community 473`, `Community 346`, `Community 2267`, `Community 477`, `Community 2295`, `Community 2296`, `Community 2297`, `Community 2298`, `Community 2299`, `Community 2300`, `Community 2301`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `Event Sourcing & Domain Events` to `Article Pipeline`, `Community 66`, `Community 482`, `Widgets & Calculator`, `Community 328`, `Redis & Caching`, `Community 473`, `Community 46`, `Chat UI Components`, `Community 57`, `Community 346`, `Community 316`, `Community 477`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `User` connect `Neuro Memory & LTM` to `Community 321`, `Event Sourcing & Domain Events`, `Community 130`, `Community 484`, `Community 70`, `Article Detection & Styles`, `Community 40`, `Redis & Caching`, `Model Loading Tests`, `Community 331`, `Community 79`, `Community 49`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Are the 433 inferred relationships involving `PipelineState` (e.g. with `PipelineOrchestrator` and `PreflightDecision`) actually correct?**
  _`PipelineState` has 433 INFERRED edges - model-reasoned connections that need verification._
- **Are the 151 inferred relationships involving `WorkflowServices` (e.g. with `CriticDimensionScore` and `FileSearchPort`) actually correct?**
  _`WorkflowServices` has 151 INFERRED edges - model-reasoned connections that need verification._
- **Are the 133 inferred relationships involving `ProviderRouter` (e.g. with `PipelineOrchestrator` and `PreflightDecision`) actually correct?**
  _`ProviderRouter` has 133 INFERRED edges - model-reasoned connections that need verification._
- **Are the 75 inferred relationships involving `DomainEvent` (e.g. with `Aggregate` and `PipelineAggregate`) actually correct?**
  _`DomainEvent` has 75 INFERRED edges - model-reasoned connections that need verification._