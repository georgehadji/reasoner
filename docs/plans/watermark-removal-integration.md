...
**2. Stripping C2PA from images Reasoner itself just generated** is the sharpest case: Reasoner requests an image from Gemini/OpenAI, the provider attaches provenance, and Reasoner removes it before delivery. That makes Reasoner the party that removed the mark, on content it generated. Hence `WATERMARK_IMAGE_STRIP_GENERATED=false` by default — opt-in, per-run, audit-logged. **Update 2026-08-19: the operator flipped this default to `true`, overriding the recommendation in this section. The analysis above still stands and is the reason to revisit it before deploying into a jurisdiction with provider marking duties.** **Uploaded** images are the opposite case (the user's own file, EXIF GPS is a genuine privacy leak) and default to on.
