...
**Status:** in progress — §2a and tier T1 complete (#47, #49); F1, F2, F3, P1, O1, O2, S1 not started
**Date:** 2026-09-02
...
### 6a. T1 results (executed 2026-09-02/03, merged as #49)

Status: **complete for sweeps 1–3, partial for sweep 4.** Four confirmed defects, each proven by trigger.

| Sweep | Result |
|---|---|
| 1. Type fidelity | 1 confirmed: `tests/test_saas_quota_repo.py` returned `str` for a `uuid` column. Reintroducing the original bug now fails 2 of 6; before, it failed nothing. Two look-alike sites cleared as innocent (`test_saas_cached_quota.py` fakes a Redis payload, where `str` is faithful; `cached_quota_repo.py:40` wraps genuine JSON) |
| 2. Global teardown | 0 confirmed. Every `set_*_port` call site and module-global assignment checked. `_MAX_KEYS` / `_MAX_ENTRIES` writes are instance attributes shadowing class attributes. `test_saas_stripe_adapter.py`'s fixture does leak three env vars, but `settings` reads them at import and no test asserts their absence, so no trigger fires — hygiene, not a defect |
| 3. Vacuity | 3 confirmed, all the same shape: `assert A or B` with `B` unconditionally true. `test_vs_observability.py` (`or store.size == 21`), `test_article_golden_set.py` (`or len(prompt) > 0`, and the left side named a local variable that is never emitted), `test_vs_coverage_audit.py` (a four-way disjunction exhausting a three-member enum plus the empty case) |
| 4. Order dependence | **Partial.** 4067 passed / 83 skipped / 4 xfailed, identical at `-n 4` and `-n 8`. No dependence surfaced |

**Sweep 3's second-order finding.** `test_vs_coverage_audit.py::test_all_three_gap_types` did not merely assert weakly, it named something unreachable: `audit_claim_coverage_vs` branches only on `overlap < 0.5` (GENUINE) and `< 0.9` (PHRASING_MISMATCH), so `GapType.COVERED` is never appended by any input. Rewritten to cover the two reachable types, with a second test pinning `COVERED` as unemitted. Whether the enum member should exist at all is a design call left open.

**Sweep 4 is weaker than this plan assumed, and the reason is a finding in itself.** The plan prescribes `-p no:randomly` on and off. `pytest-randomly` is **not installed**, so that flag was always a no-op and test order has never been randomized in this repo. `pytest.ini` sets `--dist loadscope`, which pins each module to one worker in file order. Changing the worker count therefore perturbs only which modules share a worker — it cannot reorder tests within a module. Two configurations passing is real but limited evidence: intra-module order dependence remains **UNKNOWN**, and that is the class the `_SHARED_CACHE_PORT` leak belonged to.

Two follow-ups, neither taken here because each is a judgment call rather than a fix:

- Adding `pytest-randomly` to `requirements-dev.txt` would make sweep 4 meaningful, at the cost of a suite whose order changes per run. That trade is the maintainer's to make.
- The `tests/README` rule from the deliverable above is not written yet.
