...
> **Staleness, checked 2026-09-08.** Sections 1-6 are an external snapshot of
> OpenRouter as it stood on the research date and are not re-verified here;
> read them as "what was true in April", not as current pricing. Section 7 makes
> claims about *this repository*, so it was audited against
> `src/reasoner/infrastructure/llm/registry.py` on 2026-09-08 and its first
> table was replaced. Nine of that table's thirteen rows were wrong: five named
> aliases the registry no longer has at all. The registry is the single source
> of truth for what alias serves what model. Do not hand-maintain a copy of it
> here again.
...
The table that stood here mirrored `_MODEL_WHITELIST` by hand and had rotted:
audited 2026-09-08, nine of thirteen rows disagreed with the registry.

| Row from the April snapshot | Registry on 2026-09-08 |
|---|---|
| `claude-sonnet` -> `anthropic/claude-sonnet-5` | unchanged |
| `kimi-k2-5` -> `moonshotai/kimi-k2.5` | unchanged |
| `kimi-k2-6` -> `moonshotai/kimi-k2.6` | unchanged |
| `claude-opus` -> `qwen/qwen3.6-plus` | now `anthropic/claude-opus-5` |
| `gpt-5` -> `openai/gpt-5.4` | serves `openai/gpt-5`; `gpt-5.4` is its own alias |
| `gpt-5-mini` -> `openai/gpt-5.4-mini` | serves `openai/gpt-5-mini`; `gpt-5.4-mini` is its own alias |
| `gemini-pro` -> `google/gemini-2.5-pro` | alias removed. It never served that path; `registry.py` records it serving `anthropic/claude-sonnet-5`. Use `gemini-pro-real` |
| `gemini-flash` -> `google/gemini-2.5-flash` | alias removed. `gemini-2.5-flash` is the key |
| `glm-5`, `glm-5.1`, `glm-4-airx` | all removed; the GLM aliases are `glm-5.2`, `glm-5.3`, `glm-5.3-flash` |
| `grok-4`, `grok-4.20` | both removed; the text Grok aliases are `grok-4.3`, `grok-4.5`, `grok-4.6` |

To ask the registry directly instead of trusting any table:

```bash
PYTHONPATH=src python -c "from reasoner.infrastructure.llm.registry import _MODEL_WHITELIST as W; [print(f'{k:24s} -> {v[\"model\"]}') for k, v in sorted(W.items())]"
```
