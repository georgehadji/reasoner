...
async def run_gate_cached(gate: HyperGateAgent, problem: str) -> GateDecision:
    ...
    # JEV_MODE=shadow only (in active mode jev already ran inside decide()).
    # Background, never awaited, never changes `decision`. Cache hits above
    # skip it: they repeat a problem the shadow has already seen.
    jev_router.schedule(problem, decision)
